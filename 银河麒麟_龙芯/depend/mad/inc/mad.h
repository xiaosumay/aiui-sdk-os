...
#    define MAD_VERSION                                                                     \
        ...
        "." MAD_VERSION_STRING(MAD_VERSION_MINOR) "." MAD_VERSION_STRING(MAD_VERSION_PATCH) \
            MAD_VERSION_EXTRA
...
#            define MAD_F(x)                                                   \
                ((mad_fixed_t)(((x##L) + (1L << (28 - MAD_F_FRACBITS - 1))) >> \
                               (28 - MAD_F_FRACBITS)))
...
#            define mad_f_mul(x, y)                                                            \
                ((mad_fixed_t)((((mad_fixed64_t)(x) * (y)) + (1L << (MAD_F_SCALEBITS - 1))) >> \
                               MAD_F_SCALEBITS))
#        else
...
#            define MAD_F_MLX(hi, lo, x, y) \
                asm("imull %3" : "=a"(lo), "=d"(hi) : "%a"(x), "rm"(y) : "cc")
...
#                define mad_f_scale64(hi, lo)                                              \
                    ({                                                                     \
                        ...
                        asm("addl %4,%2\n\t"                                               \
                            "adcl %5,%3"                                                   \
                            : "=rm"(__lo_), "=rm"(__hi_)                                   \
                            : "0"(lo), "1"(hi), "ir"(1L << (MAD_F_SCALEBITS - 1)), "ir"(0) \
                            : "cc");                                                       \
                        asm("shrdl %3,%2,%1"                                               \
                            : "=rm"(__result)                                              \
...
#            define mad_f_mul(x, y)                                                        \
                ({                                                                         \
                    ...
                    asm("smull	%0, %1, %3, %4\n\t"                                         \
                        ...
                        : "%r"(x), "r"(y), "M"(MAD_F_SCALEBITS), "M"(32 - MAD_F_SCALEBITS) \
                        : "cc");                                                           \
                    __result;                                                              \
                })
#        endif
...
#        define MAD_F_MLX(hi, lo, x, y) \
            asm("smull	%0, %1, %2, %3" : "=&r"(lo), "=&r"(hi) : "%r"(x), "r"(y))

#        define MAD_F_MLA(hi, lo, x, y) \
            asm("smlal	%0, %1, %2, %3" : "+r"(lo), "+r"(hi) : "%r"(x), "r"(y))
...
#        define mad_f_scale64(hi, lo)                                                   \
            ({                                                                          \
                ...
                asm("movs	%0, %1, lsr %3\n\t"                                           \
                    "adc	%0, %0, %2, lsl %4"                                            \
                    : "=&r"(__result)                                                   \
                    : "r"(lo), "r"(hi), "M"(MAD_F_SCALEBITS), "M"(32 - MAD_F_SCALEBITS) \
                    : "cc");                                                            \
                __result;                                                               \
            })
...
#            define MAD_F_ML0(hi, lo, x, y) \
                asm("mult	%2,%3" : "=l"(lo), "=h"(hi) : "%r"((x) >> 12), "r"((y) >> 16))
#            define MAD_F_MLA(hi, lo, x, y) \
                asm("madd16	%2,%3" : "+l"(lo), "+h"(hi) : "%r"((x) >> 12), "r"((y) >> 16))
...
#            define mad_f_scale64(hi, lo)                                                      \
                ({                                                                             \
                    ...
                    asm("rotrwi %0,%1,%2" : "=r"(__result) : "r"(lo), "i"(MAD_F_SCALEBITS));   \
                    asm("extrwi %0,%1,1,0" : "=r"(__round) : "r"(__result));                   \
                    asm("insrwi %0,%1,%2,0" : "+r"(__result) : "r"(hi), "i"(MAD_F_SCALEBITS)); \
                    asm("add %0,%1,%2" : "=r"(__result) : "%r"(__result), "r"(__round));       \
                    __result;                                                                  \
                })
#        else
#            define mad_f_scale64(hi, lo)                                                      \
...
void mad_timer_string(
    mad_timer_t, char *, char const *, enum mad_units, enum mad_units, unsigned long);
...
#    define MAD_NSBSAMPLES(header)                                                              \
        ((header)->layer == MAD_LAYER_I                                                         \
             ? 12                                                                               \
             : (((header)->layer == MAD_LAYER_III && ((header)->flags & MAD_FLAG_LSF_EXT)) ? 18 \
                                                                                           : 36))
